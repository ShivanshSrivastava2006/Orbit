...
export default function OtpScreen() {
  ...
  const sendOtp = async () => {
    try {
      const res = await fetch("http://192.168.0.185:5050/send-otp", { // ⬅️ edit YOUR IP ADDRESS HERE
        method: "POST",
  ...
  const verifyOtp = async () => {
    try {
      const res = await fetch("http://192.168.0.185:5050/verify-otp", { // ⬅️ edit YOUR IP ADDRESS HERE
        method: "POST",
        headers: { "Content-Type": "application/json" },
